...
- Preserve lattice volume in Delaunay reduction when the three shortest candidate vectors are coplanar, as can occur for elongated cells (#470).
...
- Forward the warning about omitted incompatible operations to Python's `logging` system at `WARNING` level. Dataset construction succeeds and returns only compatible operations (#446).
...
- Fix monoclinic standardized cells (mC in particular) coming out with `beta` close to 180 deg, e.g. `a' = 2c - a` with `beta` = 172 deg where the reduced setting has `beta` = 91.6 deg (#370). The candidate changes of basis are now built from the Delaunay-reduced triple of the plane perpendicular to the unique axis instead of a bounded `[-1, 1]` brute force, so `90 <= beta <= 120` deg, and ties are broken by the lexicographically shortest `(a, b, c)`, giving `a <= c` where the setting allows the `a <-> c` swap. The 60 seekpath band-path structures now give the same standardized and primitive cells as spglib with `Setting::Spglib` (#427)
