...
pub fn swapping_column_matrix<D: DimName>(col1: usize, col2: usize) -> OMatrix<i32, D, D>
where
    DefaultAllocator: Allocator<i32, D, D>,
{
    let mut trans_mat = OMatrix::<i32, D, D>::zeros();
    let dim = D::dim();
    for i in 0..dim {
        if i == col1 {
...
pub fn adding_column_matrix<D: DimName>(col1: usize, col2: usize, k: i32) -> OMatrix<i32, D, D>
where
    DefaultAllocator: Allocator<i32, D, D>,
{
    let mut trans_mat = OMatrix::<i32, D, D>::identity();
    let dim = D::dim();
    for i in 0..dim {
        if i == col1 {
...
pub fn changing_sign_matrix<D: DimName>(col: usize) -> OMatrix<i32, D, D>
where
    DefaultAllocator: Allocator<i32, D, D>,
{
    let mut trans_mat = OMatrix::<i32, D, D>::identity();
...
mod tests {
    ...
    use super::{adding_column_matrix, changing_sign_matrix, swapping_column_matrix};
    ...
    #[test]
    fn test_changing_sign_matrix() {
        let actual = changing_sign_matrix::<U3>(0);
        let expect = matrix![
            -1, 0, 0;
            0, 1, 0;
            0, 0, 1;
