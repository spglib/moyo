...
fn test_search_symmetry_operations_fcc(primitive_fcc: Cell) {
    ...
    let symprec = 1e-4;
    let result = SymmetrySearch::new(&primitive_cell, symprec, AngleTolerance::Default).unwrap();
...
fn test_search_symmetry_operations_rutile(rutile: Cell) {
    ...
    let result = SymmetrySearch::new(&primitive_cell, symprec, angle_tolerance).unwrap();
