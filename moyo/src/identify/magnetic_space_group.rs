...
/// Find a unimodular transformation that transforms `(E, src_translation)` to
/// `(E, dst_translation)` while keeping `stabilized_prim_operations`, which are
/// generated by `stabilized_prim_generators`.
///
/// This function relies on the bounded search in `iter_unimodular_trans_mat`.
/// For a reduced basis, the search window is sufficient to verify existence.
/// For a non-reduced basis, the search is only best effort and may not be exhaustive.
