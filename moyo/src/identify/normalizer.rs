...
impl Normalizer {
    ...
    pub fn operations(&self) -> Vec<UnimodularTransformation> {
        // Enumerate the discrete translation subgroup mod Z^3 generated by
        // `self.translations` by closure, deduping with a quantized key (the
        // same denominator-24 trick the tests use for crystallographic
        // fractions).
        let quantize = |t: &Translation| -> [i64; 3] {
            let denom = 24_i64;
        ...
        seen.insert(quantize(&Translation::zeros()));
        let mut frontier = vec![Translation::zeros()];
        while let Some(base) = frontier.pop() {
            for generator in &self.translations {
                let next = (base + generator).map(|e| e.rem_euclid(1.0));
                if seen.insert(quantize(&next)) {
                    discrete.push(next);
                    frontier.push(next);
                }
            }
...
mod tests {
    ...
    fn test_operations_enumeration() {
        ...
        // 8 discrete translations (the inversion-center half-translations, i.e.
        // the {0, 1/2}^3 subgroup), so 384 discrete normalizer operations.
        let hall_symbol = HallSymbol::from_hall_number(2).unwrap();
        let prim_operations = hall_symbol.primitive_traverse();
        let prim_generators = hall_symbol.primitive_generators();
        let normalizer = Normalizer::from_lattice(
