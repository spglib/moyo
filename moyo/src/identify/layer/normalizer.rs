...
/// Layer-aware sibling of [`super::super::integral_normalizer`] for the
/// (2 + 1)-dimensional layer-group convention. Mirrors the bulk function
/// with one extra filter: only conjugators in **layer-block form**
/// (`T[0,2] = T[1,2] = T[2,0] = T[2,1] = 0`, `T[2,2] = +/-1`, paper
/// Fu et al. 2024 eq. 4) are returned, so the c-axis stays the
/// aperiodic stacking direction.
///
...
///
/// Same input contract as [`super::super::integral_normalizer`]:
/// `prim_operations` is expected in a reduced basis; the result is
/// best-effort within `iter_unimodular_trans_mat`'s bounded search.
